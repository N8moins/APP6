...
def probleme1():
    ...
    s1, s11 = signal.zpk2tf(z1, p1, k1)
    s2, s22 = signal.zpk2tf(z2, p2, k2)

    #Ajouter le code pour generer la carte des poles et zeros et le lieu de Bode
    #utiliser les fonctions dans helpers.py
    helpers.bodeplot(s1, s11, 'Lieu de bode H1')
    helpers.bodeplot(s2, s22, 'Lieu de bode H2')
    ...
    for i in range(len(w)):
        ...
        # temp = signal.lsim((B, A), u[i], A)  # temp = [t, y, x], voir l'aide de lsim
        tout.append(temp[0])
        yout1.append(temp[1])

        temp = signal.lsim(t, u, i)  # temp = [t, y, x], voir l'aide de lsim
        yout2.append(temp[1])
...
def probleme2():
    ...
    num1, den1 = signal.butter(2, wc, 'low', analog=True)
    z1, p1, k1 = signal.tf2zpk(num1, den1)

    num2, den2 = signal.butter(2, wc, 'high', analog=True)
    z2, p2, k2 = signal.tf2zpk(num2, den2)


    #Affichage des polynomes
    print(f'Passe-bas Numérateur {num1}, Dénominateur {den1}')  # affiche les coefficients correspondants au filtre
    print(f'Passe-haut Numérateur {num2}, Dénominateur {den2}')  # affiche les coefficients correspondants au filtre
    ...
    bp1, ap1 = signal.zpk2tf(zp,pp,kp)

    toutp, youtp, xoutp = signal.lsim((bp1, ap1), u1, t)
    tout1, yout1, xout1 = signal.lsim((z1, p1, k1 * lowk), u1, t)
    tout2, yout2, xout2 = signal.lsim((z2, p2, k2 * highk), u1, t)
    yout = [yout1, yout2, youtp]

    hp.timepltmulti2(t, u1, toutp, yout, f'Égaliseur klow={lowk}, khigh={highk}', ['H1', 'H2',
                                                                                     'HÉgaliseur'])
    #Question D
    ...
    toutp, youtp, xoutp = signal.lsim((bp1, ap1), u2, t)
    tout1, yout1, xout1 = signal.lsim((z1, p1, k1 * lowk), u2, t)
    tout2, yout2, xout2 = signal.lsim((z2, p2, k2 * highk), u2, t)
    yout = [yout1, yout2, youtp]

    hp.timepltmulti2(t, u2, toutp, yout, f'Égaliseur klow={lowk}, khigh={highk}', ['H1', 'H2',
                                                                                   'HÉgaliseur'])
